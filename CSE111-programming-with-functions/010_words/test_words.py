...
def test_suffix():
    ...
    assert suffix("", "") == ""           # Esperado: ""
    assert suffix("", "correct") == ""    # Esperado: ""
    assert suffix("clear", "") == ""      # Esperado: ""
    assert suffix("angelic", "awesome") == ""  # Esperado: ""
    assert suffix("found", "profound") == "found"  # Esperado: "found"
    assert suffix("ditch", "itch") == "itch"      # Esperado: "itch"
    assert suffix("happy", "funny") == "y"        # Esperado: "y"
    assert suffix("tired", "fatigued") == "ed"    # Esperado: "ed"
    assert suffix("swimming", "FLYING") == "ing"  # Esperado: "ing"
